...
let K=new Array(200).fill(0), kx=new Array(200).fill(0), ky=new Array(200).fill(0), kh=new Array(200).fill(1);
...
function draw() {
  ...
  if(state_G==0){
    ...
    if(hp==300){
      ...
      K=new Array(200).fill(0); kx=new Array(200).fill(0); ky=new Array(200).fill(0); kh=new Array(200).fill(1);
...
function poi(){
  ...
  for(let i=0; i<kl; i++){
     let tK=random(0, 20);
     if((kx[i]-mouseX)*(kx[i]-mouseX)+(ky[i]-mouseY)*(ky[i]-mouseY)<151 && state_S==1){
       pt-=kh[i]/10;
       hp-=kh[i]/10;
       K[i]=0;
       ky[i]=-40;
     ...
     if(slhair.value()==1){
        for(let j=0; j<10; j++){
          if(Bullet1[j]==1 && (kx[i]-Bullet1x[j])*(kx[i]-Bullet1x[j])+(ky[i]-Bullet1y[j])*(ky[i]-Bullet1y[j])<151){
            kh[i]-=30;
            Bullet1[j]=0;
          }
        }
        ...
        for(let j=0; j<30; j++){
          if(Bullet2[j]==1 && (kx[i]-Bullet2x[j])*(kx[i]-Bullet2x[j])+(ky[i]-Bullet2y[j])*(ky[i]-Bullet2y[j])<151){
            kh[i]-=10;
            Bullet2[j]=0;
          }
        }
        ...
        for(let j=0; j<50; j++){
          if(Bullet3[j]==1 && ((kx[i]-Bullet3x[j])*(kx[i]-Bullet3x[j])+(ky[i]-Bullet3y[j])*(ky[i]-Bullet3y[j])<151 || (kx[i]-Bullet3x[j]-x3shift[j])*(kx[i]-Bullet3x[j]-x3shift[j])+(ky[i]-Bullet3y[j])*(ky[i]-Bullet3y[j])<151 || (kx[i]-Bullet3x[j]+x3shift[j])*(kx[i]-Bullet3x[j]+x3shift[j])+(ky[i]-Bullet3y[j])*(ky[i]-Bullet3y[j])<151)){
            kh[i]-=8;
            Bullet3[j]=0;
          }
        }
      ...
      if(tK<1 && K[i]==0){
        K[i]=1;
        ky[i]=random(0, windowHeight);
        kh[i]=1000;
      }
      if(K[i]==1){
        if(kx[i]<windowWidth+40){
          if(state_S==1){
            if(sllevel.value()>=4){
              let ra=random(0, 5);
              if(ra<1){
  ...
  for(let i=0; i<7; i++){
      ...
      text(pt, 130, 40);
      pop();
      if((bx[i]-mouseX)*(bx[i]-mouseX)+(by[i]-mouseY)*(by[i]-mouseY)<151 && state_S==1){
        pt-=bh[i];
        hp-=bh[i];
        B[i]=0;
        by[i]=-40;
      ...
      if(slhair.value()==1){
        for(let j=0; j<10; j++){
          if(Bullet1[j]==1 && (bx[i]-Bullet1x[j])*(bx[i]-Bullet1x[j])+(by[i]-Bullet1y[j])*(by[i]-Bullet1y[j])<151){
            bh[i]-=30;
            Bullet1[j]=0;
          }
        }
        ...
        for(let j=0; j<30; j++){
          if(Bullet2[j]==1 && (bx[i]-Bullet2x[j])*(bx[i]-Bullet2x[j])+(by[i]-Bullet2y[j])*(by[i]-Bullet2y[j])<151){
            bh[i]-=10;
            Bullet2[j]=0;
          }
        }
        ...
        for(let j=0; j<50; j++){
          if(Bullet3[j]==1 && ((bx[i]-Bullet3x[j])*(bx[i]-Bullet3x[j])+(by[i]-Bullet3y[j])*(by[i]-Bullet3y[j])<151 || (bx[i]-Bullet3x[j]-x3shift[j])*(bx[i]-Bullet3x[j]-x3shift[j])+(by[i]-Bullet3y[j])*(by[i]-Bullet3y[j])<151 || (bx[i]-Bullet3x[j]+x3shift[j])*(bx[i]-Bullet3x[j]+x3shift[j])+(by[i]-Bullet3y[j])*(by[i]-Bullet3y[j])<151)){
            bh[i]-=8;
            Bullet3[j]=0;
          }
        }
  ...
  if(sllevel.value()>=2){
    for(let i=0; i<13; i++){
      let tC=random(0, 20);
      if((cx[i]-mouseX)*(cx[i]-mouseX)+(cy[i]-mouseY)*(cy[i]-mouseY)<151 && state_S==1){
        pt-=ch[i];
        hp-=ch[i];
        C[i]=0;
        cy[i]=-40;
      ...
      if(slhair.value()==1){
        for(let j=0; j<10; j++){
          if(Bullet1[j]==1 && (cx[i]-Bullet1x[j])*(cx[i]-Bullet1x[j])+(cy[i]-Bullet1y[j])*(cy[i]-Bullet1y[j])<151){
            ch[i]-=30;
            Bullet1[j]=0;
          }
        }
        ...
        for(let j=0; j<30; j++){
          if(Bullet2[j]==1 && (cx[i]-Bullet2x[j])*(cx[i]-Bullet2x[j])+(cy[i]-Bullet2y[j])*(cy[i]-Bullet2y[j])<151){
            ch[i]-=10;
            Bullet2[j]=0;
          }
        }
        ...
        for(let j=0; j<50; j++){
          if(Bullet3[j]==1 && ((cx[i]-Bullet3x[j])*(cx[i]-Bullet3x[j])+(cy[i]-Bullet3y[j])*(cy[i]-Bullet3y[j])<151 || (cx[i]-Bullet3x[j]-x3shift[j])*(cx[i]-Bullet3x[j]-x3shift[j])+(cy[i]-Bullet3y[j])*(cy[i]-Bullet3y[j])<151 || (cx[i]-Bullet3x[j]+x3shift[j])*(cx[i]-Bullet3x[j]+x3shift[j])+(cy[i]-Bullet3y[j])*(cy[i]-Bullet3y[j])<151)){
            ch[i]-=8;
            Bullet3[j]=0;
          }
        }
  ...
  if(sllevel.value()>=3){
    for(let i=0; i<20; i++){
      let tD=random(0, 20);
      if((dx[i]-mouseX)*(dx[i]-mouseX)+(dy[i]-mouseY)*(dy[i]-mouseY)<151 && state_S==1){
        pt-=dh[i];
        hp-=dh[i];
        D[i]=0;
        dy[i]=-40;
      ...
      if(slhair.value()==1){
        for(let j=0; j<10; j++){
          if(Bullet1[j]==1 && (dx[i]-Bullet1x[j])*(dx[i]-Bullet1x[j])+(dy[i]-Bullet1y[j])*(dy[i]-Bullet1y[j])<151){
            dh[i]-=30;
            Bullet1[j]=0;
          }
        }
        ...
        for(let j=0; j<30; j++){
          if(Bullet2[j]==1 && (dx[i]-Bullet2x[j])*(dx[i]-Bullet2x[j])+(dy[i]-Bullet2y[j])*(dy[i]-Bullet2y[j])<151){
            dh[i]-=10;
            Bullet2[j]=0;
          }
        }
        ...
        for(let j=0; j<50; j++){
          if(Bullet3[j]==1 && ((dx[i]-Bullet3x[j])*(dx[i]-Bullet3x[j])+(dy[i]-Bullet3y[j])*(dy[i]-Bullet3y[j])<151 || (dx[i]-Bullet3x[j]-x3shift[j])*(dx[i]-Bullet3x[j]-x3shift[j])+(dy[i]-Bullet3y[j])*(dy[i]-Bullet3y[j])<200 || (dx[i]-Bullet3x[j]+x3shift[j])*(dx[i]-Bullet3x[j]+x3shift[j])+(dy[i]-Bullet3y[j])*(dy[i]-Bullet3y[j])<200)){
            dh[i]-=8;
            Bullet3[j]=0;
          }
        }
  ...
  if(sllevel.value()>=4){
    ...
    for(let i=0; i<k; i++){
      let tE=random(0, 20);
      if((ex[i]-mouseX)*(ex[i]-mouseX)+(ey[i]-mouseY)*(ey[i]-mouseY)<151 && state_S==1){
        pt-=eh[i];
        hp-=eh[i];
        E[i]=0;
        ey[i]=-40;
      ...
      if(slhair.value()==1){
        for(let j=0; j<10; j++){
          if(Bullet1[j]==1 && (ex[i]-Bullet1x[j])*(ex[i]-Bullet1x[j])+(ey[i]-Bullet1y[j])*(ey[i]-Bullet1y[j])<151){
            eh[i]-=30;
            Bullet1[j]=0;
          }
        }
        ...
        for(let j=0; j<30; j++){
          if(Bullet2[j]==1 && (ex[i]-Bullet2x[j])*(ex[i]-Bullet2x[j])+(ey[i]-Bullet2y[j])*(ey[i]-Bullet2y[j])<151){
            eh[i]-=10;
            Bullet2[j]=0;
          }
        }
        ...
        for(let j=0; j<50; j++){
          if(Bullet3[j]==1 && ((ex[i]-Bullet3x[j])*(ex[i]-Bullet3x[j])+(ey[i]-Bullet3y[j])*(ey[i]-Bullet3y[j])<151 || (ex[i]-Bullet3x[j]-x3shift[j])*(ex[i]-Bullet3x[j]-x3shift[j])+(ey[i]-Bullet3y[j])*(ey[i]-Bullet3y[j])<151 || (ex[i]-Bullet3x[j]+x3shift[j])*(ex[i]-Bullet3x[j]+x3shift[j])+(ey[i]-Bullet3y[j])*(ey[i]-Bullet3y[j])<151)){
            eh[i]-=8;
            Bullet3[j]=0;
          }
        }
